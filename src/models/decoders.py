...
class MultiSessionMLPDecoder(BaselineMultiSessionDecoder):
    def __init__(self, config):
        ...
        self.flat_layer = torch.nn.Linear(self.hidden_size[0]*self.n_t_steps, self.hidden_size[0])
            
        # Hidden layers
        for l in range(len(self.hidden_size)-1):
            self.hidden_layers.append(torch.nn.Linear(self.hidden_size[l], self.hidden_size[l+1]))
            self.hidden_layers.append(torch.nn.ReLU())
            self.hidden_layers.append(torch.nn.Dropout(self.drop_out))
