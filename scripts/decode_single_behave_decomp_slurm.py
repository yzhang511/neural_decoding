...
for comp_idx in range(9, args.n_pc_components):
    ...
    for model_type in ['ridge', 'reduced-rank', 'lstm', 'mlp']:
        ...
        if model_type == "reduced-rank":
            ...
            search_space['tune_max_epochs'] = 500
            search_space['learning_rate'] = 1e-2
            search_space['max_epochs'] = 500
        elif model_type == "lstm":
            search_space = base_config.copy()
            search_space['lstm_n_layers'] =  3
            search_space['lstm_hidden_size'] = tune.grid_search([128, 64])
            search_space['mlp_hidden_size'] = tune.grid_search([(64,), (32,)])
            search_space['drop_out'] = tune.grid_search([0., 0.2, 0.4, 0.6])
            search_space['tune_max_epochs'] =  100
            search_space['max_epochs'] = 250
        elif model_type == "mlp":
            search_space = base_config.copy()
            search_space['mlp_hidden_size'] = (256, 128, 64)
            search_space['drop_out'] = tune.grid_search([0., 0.2, 0.4, 0.6])
            search_space['tune_max_epochs'] = 200
            search_space['max_epochs'] = 250
        else:
            raise NotImplementedError
        ...
        results = tune_decoder(
            ...
            num_samples=search_space['tune_n_samples'], num_workers=args.n_workers
        )
        ...
        _, test_pred, test_y = eval_model(dm.train, dm.test, model, model_type=model_type, plot=False)
        save_results(model_type, r2, test_pred, test_y)
